...
class CUNE_BPR(RankingRecommender):
    def __init__(self, sess, data, configs, logger):
        ...
        self.data.user_friends, self.SPu = get_topK_friends_and_SPu(data, self.walk_count, self.walk_length, self.walk_dim, self.window_size, self.topK_f)
        # Specify training model
        self.train_model = partial(self.train_model_sbpr, is_suk=False)

    def _create_inputs(self):
        def __create_p(dtype_, shape_, name_):
            return tf.placeholder(dtype_, shape=shape_, name=name_)
        with tf.name_scope('cune_inputs'):
    ...
    def _create_params(self):
        ...
        with tf.variable_scope('cune_params'):
            # Embedding matrix
    ...
    def _save_model(self):
        var_list = {'cune_params/P': self.P, 'cune_params/Q': self.Q, 'cune_params/bias': self.bias}
